...
LiburingChannel& LiburingEnv::getIoChannel(int channel) {
   ...
   if (ch == channels.end()) {
      // create new
      ch = channels.insert({channel, std::make_unique<LiburingChannel>(*raidCtl, ioOptions, *this)}).first;
   }
   return *ch->second;
}
// -------------------------------------------------------------------------------------
// Channel
// -------------------------------------------------------------------------------------
LiburingChannel::LiburingChannel(RaidController<int>& raidCtl, const IoOptions& ioOptions, LiburingEnv& env) : LinuxBaseChannel(raidCtl, ioOptions) {
   struct nvme_id_ns ns;
   ...
   if (ioOptions.ioUringShareWq > 0 && env.channels.size() >= (unsigned)ioOptions.ioUringShareWq) {
      ...
      iouParameters.wq_fd = dynamic_cast<LiburingChannel*>(env.channels[static_cast<int>(env.channels.size() % ioOptions.ioUringShareWq)].get())->ring.ring_fd;
   }
...
// -------------------------------------------------------------------------------------
int LiburingChannel::_poll(int min) {
   int done = 0;
